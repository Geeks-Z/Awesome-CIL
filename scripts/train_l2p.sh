#!/bin/bash
#python main.py --config=./scripts/l2p/l2p_cifar_B0_Inc5.json
#python main.py --config=./scripts/l2p/l2p_cifar_B0_Inc10.json
python main.py --config=./scripts/l2p/l2p_cifar_B0_Inc20.json
#python main.py --config=./scripts/l2p/l2p_cifar_B50_Inc5.json
#python main.py --config=./scripts/l2p/l2p_cifar_B50_Inc10.json
#python main.py --config=./scripts/l2p/l2p_cub_B0_Inc5.json
#python main.py --config=./scripts/l2p/l2p_cub_B0_Inc10.json
#python main.py --config=./scripts/l2p/l2p_cub_B0_Inc20.json
#python main.py --config=./scripts/l2p/l2p_cub_B0_Inc40.json
#python main.py --config=./scripts/l2p/l2p_cub_B100_Inc5.json
#python main.py --config=./scripts/l2p/l2p_cub_B100_Inc10.json
#python main.py --config=./scripts/l2p/l2p_cub_B100_Inc20.json
#python main.py --config=./scripts/l2p/l2p_inr_B0_Inc5.json
#python main.py --config=./scripts/l2p/l2p_inr_B0_Inc10.json
#python main.py --config=./scripts/l2p/l2p_inr_B0_Inc20.json
python main.py --config=./scripts/l2p/l2p_inr_B0_Inc40.json
#python main.py --config=./scripts/l2p/l2p_inr_B100_Inc5.json
#python main.py --config=./scripts/l2p/l2p_inr_B100_Inc10.json
#python main.py --config=./scripts/l2p/l2p_inr_B100_Inc20.json
#python main.py --config=./scripts/l2p/l2p_ina_B0_Inc5.json
#python main.py --config=./scripts/l2p/l2p_ina_B0_Inc10.json
#python main.py --config=./scripts/l2p/l2p_ina_B0_Inc20.json
#python main.py --config=./scripts/l2p/l2p_ina_B0_Inc40.json
#python main.py --config=./scripts/l2p/l2p_ina_B100_Inc5.json
#python main.py --config=./scripts/l2p/l2p_ina_B100_Inc10.json
#python main.py --config=./scripts/l2p/l2p_ina_B100_Inc20.json
#python main.py --config=./scripts/l2p/l2p_omn_B0_Inc5.json
#python main.py --config=./scripts/l2p/l2p_omn_B0_Inc10.json
#python main.py --config=./scripts/l2p/l2p_omn_B0_Inc20.json
#python main.py --config=./scripts/l2p/l2p_omn_B0_Inc30.json
#python main.py --config=./scripts/l2p/l2p_omn_B150_Inc5.json
#python main.py --config=./scripts/l2p/l2p_omn_B150_Inc10.json
#python main.py --config=./scripts/l2p/l2p_omn_B150_Inc30.json
#python main.py --config=./scripts/l2p/l2p_vtab_B0_Inc5.json
#python main.py --config=./scripts/l2p/l2p_vtab_B0_Inc10.json








