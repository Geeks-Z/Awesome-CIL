...
class Attention_lora(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0.0, proj_drop=0.0, msa=None):
        super().__init__()
        if msa is None:
            msa = [0, 0, 0]
    ...
    def forward(self, x, adapt=None, prompt=None, rank_prompt=None, block_weight=None):
        ...
        if adapt is not None:
            ...
            if self.msa[0] == 1:
                q += block_weight[0] * adapt[0](x)
            if self.msa[1] == 1:
                k += block_weight[1] * adapt[1](x)
            if self.msa[2] == 1:
                v += block_weight[2] * adapt[2](x)
...
class Block(nn.Module):
    ...
    def forward(self, x, adapt=None, prompt=None, rank_prompt=None, block_weight=None):
        if self.msa_adapt:
            x = x + self.drop_path(self.attn(self.norm1(x), adapt, prompt, rank_prompt, block_weight))
            residual = x
...
class VisionTransformer(nn.Module):
    ...
    def get_new_adapter_msa(self):
        ...
        if len(self.specfic_pos) < 12:
            ...
            for pos_idx in self.adapt_pos:
                if pos_idx in self.general_pos:
                    pos = self.adapt_pos.index(pos_idx)
    ...
    def forward_train(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if self.config.vpt_on:
                embedding = self.embeddings[idx].expand(B, -1, -1)
            ...
            if self.config.ffn_adapt:
                if idx in self.adapt_pos:
                    ...
                    if self.use_block_weight and idx in self.specfic_pos:
                        ...
                        x = blk(x, self.cur_adapter[pos], prompt, rank_prompt, block_weight=self.block_weight[:, pos_spec])
                    else:
    ...
    def forward_proto(self, x, adapt_index):
        ...
        if self.config.ffn_adapt:
                ...
                for j in range(len(self.blocks)):
                    ...
                    if j in self.adapt_pos:
                        if j in self.general_pos:
                            ...
                            pos = self.specfic_pos.index(j)
                            adapt = self.adapter_list[adapt_index][pos]
                        if self.use_block_weight and j in self.specfic_pos:
                            pos_spec = self.specfic_pos.index(j)
                            block_weight = self.block_weight_list[adapt_index][:, pos_spec]
                        else:
                            block_weight = None
                        x = self.blocks[j](x, adapt, prompt, rank_prompt, block_weight)
                    else:
            ...
            for j in range(len(self.blocks)):
                x = self.blocks[j](x, adapt=None, prompt=None, rank_prompt=None, block_weight=None)
...
def _load_pretrained_split_qkv(model_name, model):
...
def vit_base_patch16_224_cllora(pretrained=False, **kwargs):
    ...
    return _load_pretrained_split_qkv("vit_base_patch16_224", model)
...
def vit_base_patch16_224_in21k_cllora(pretrained=False, **kwargs):
    ...
    return _load_pretrained_split_qkv("vit_base_patch16_224_in21k", model)
