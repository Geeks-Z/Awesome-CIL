...
from backbone import vit_inflora
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    default_num_classes = _cfg_value(pretrained_cfg, "num_classes")
    ...
    model = build_model_with_cfg(
        ...
        pretrained_custom_load="npz" in _cfg_value(pretrained_cfg, "url"),
        **kwargs
    )
    return model
...
def _normalize_backbone_name(name):
    ...
    if name.endswith("_inflora"):
        name = name[: -len("_inflora")]
    return aliases.get(name, name)


def _build_inflora_vit(args):
    variant = _normalize_backbone_name(
        args.get("backbone_type", "vit_base_patch16_224_in21k")
    )
    variant_specs = {
        "vit_base_patch16_224_in21k": dict(
            patch_size=16, embed_dim=768, depth=12, num_heads=12
        ),
    }
    model_kwargs = variant_specs.get(variant)
    if model_kwargs is None:
        raise NotImplementedError(
            "Unknown InfLoRA backbone type: {}".format(
                args.get("backbone_type", variant)
            )
        )

    model_kwargs.update(
        num_classes=0, n_tasks=args["total_sessions"], rank=args["rank"]
    )
    return _create_vision_transformer(
        variant, pretrained=args.get("pretrained", True), **model_kwargs
    )
...
class SiNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.image_encoder = _build_inflora_vit(args)
        ...
        self.classifier_pool = nn.ModuleList(
            [
                nn.Linear(self.image_encoder.out_dim, self.class_num, bias=True)
