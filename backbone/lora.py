...
class _LoRA_qkv_timm_train(nn.Module):
    def __init__(
        ...
        linear_b_q,
        linear_a_v,
        linear_b_v,  # linear_a_q1, linear_b_q1, linear_a_v1, linear_b_v1,
        task_id,
        saved_A,
        saved_B,
        t_layer_i,
    ...
    def forward(self, x):
        ...
        for i in range(self.task_id):
            # for i in range(0):
            saved_A_i, saved_B_i = (
                self.saved_A["saved_A_" + str(i)],
                self.saved_B["saved_B_" + str(i)],
            )
            Q, V = list(enumerate(zip(saved_A_i, saved_B_i)))[
                self.t_layer_i * 2 : self.t_layer_i * 2 + 2
            ]
            _, (A_q, B_q) = Q
            _, (A_v, B_v) = V
            ...
            if i == 0:
                new_q = self.scaling_factor_prev[i](
                    w_b_linear_q(w_a_linear_q(x))
                    / (
                        torch.norm(w_b_linear_q.weight)
                        * torch.norm(w_a_linear_q.weight)
                    )
                )
                new_v = self.scaling_factor_prev[i](
                    w_b_linear_v(w_a_linear_v(x))
                    / (
                        torch.norm(w_b_linear_v.weight)
                        * torch.norm(w_a_linear_v.weight)
                    )
                )
            else:
...
class LoRA_ViT_timm(nn.Module):
    def __init__(
        ...
        self.wrapped_param_prev = nn.ModuleList(
            [ParameterWrapper(nn.Parameter(torch.Tensor([0.8]))) for _ in range(20)]
        )
    ...
    def reset_lora_vit_head(self):
        ...
        head_device = next(self.lora_vit.parameters()).device
        self.lora_vit.head = self.generate_fc(
            768, (self.task_id) * task_incremental
        ).to(head_device)
        temp_weights = torch.load(
            self.save_file + "CLs_weight" + str(self.task_id - 1) + ".pt"
        )
        temp_bias = torch.load(
            self.save_file + "CLs_bias" + str(self.task_id - 1) + ".pt"
        )

        self.lora_vit.head.weight.data = temp_weights.data.to(head_device)
        self.lora_vit.head.bias.data = temp_bias.data.to(head_device)
    ...
    def save_wrap_param(self, filename):
        if self.task_id == 1:
            ...
            scaling_param = torch.load(
                filename + "scaling_factor" + str(self.task_id - 2) + ".pt"
            )
        i = self.task_id - 1
        # print('save i', i)
        for j in range(i + 1):
        ...
        torch.save(
            scaling_param, filename + "scaling_factor" + str(self.task_id - 1) + ".pt"
        )
    ...
    def compute_ortho_loss(self):
        loss = torch.tensor(0.0, device=self.w_As[0].weight.device)
        ...
        for i in range(self.task_id):
            ...
            if os.path.exists(file_path):
                ...
                for j in range(num_layer):
                    temp = torch.matmul(
                        w_As[j].weight.to(self.w_As[j].weight.device),
                        self.w_As[j].weight.t(),
                    )
