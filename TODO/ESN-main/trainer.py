...
def training(args, prototype_mode, taskid, train_loader, test_loader, known_classes=0, vitprompt=None, numclass=10, total_classes = 0, logger=None):
    trainer = pl.Trainer(default_root_dir='./checkpoints/'+wandb.run.name, accelerator="gpu", devices=1,
                         max_epochs=args.max_epochs, logger = logger,callbacks=[ModelCheckpoint(save_weights_only=True, mode="max", monitor="train_acc"),
...
def eval(args, load_path, datamanage, cls2task=None):
    ...
    for taskid in range(datamanage.nb_tasks):
        ...
        for _, (_, inputs, targets) in enumerate(test_till_now_loader):
            ...
            id_pred.append(torch.tensor([cls2task[v] for v in outputs.cpu().numpy()]))
            id_true.append(torch.tensor([cls2task[v] for v in targets.cpu().numpy()]))


        y_pred = np.concatenate(y_pred)
        y_true = np.concatenate(y_true)
        id_pred = np.concatenate(id_pred)
        id_true = np.concatenate(id_true)
        if args.dil:
            ...
            id_accs.append(np.around((id_pred == id_true).sum() * 100 / len(id_true), decimals=2))
    ...
    print(accs)
    print("Classify Average Accuracy: {:.2f}".format(sum(accs) / len(accs)))
    print("ID Accuracy curve:")
    print(id_accs)
    print("ID Average Accuracy: {:.2f}".format(sum(id_accs) / len(id_accs)))
