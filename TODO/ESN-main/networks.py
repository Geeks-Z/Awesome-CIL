...
from models.vit import VisionTransformer, PatchEmbed, Block, resolve_pretrained_cfg, build_model_with_cfg, \
...
class incremental_vitood(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 记录epoch粒度的loss（自动汇总为一个epoch的平均值）
        self.log(
            self.logger.experiment.config['dataset'] +"_cur_task_"+str(task_id)+ "_train_loss_epoch",
            loss,
            on_step=False,  # 不记录每一步
            on_epoch=True,  # 记录整个epoch的平均值
            prog_bar=True,  # 在进度条显示
            logger=True  # 同步到WandB
        )
        return loss
